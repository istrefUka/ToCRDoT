...
export class Project {
  ...
  constructor(projectUUID: uuid, creator: uuid, displayNameCreator: string, title: string) {
    this.append_only_log = new AppendOnlyLog();
    ...
    let operation: Operation = {
      command: "constructor",
      args: [projectUUID, creator, displayNameCreator, title]
    };
  ...
  changeName( personUuid: uuid, newName: string, append_only_log?: AppendOnlyLog): void {
    ...
    let dependencies: uuid[] = [this.projectUUID,personUuid]; //Welches nehmen? oder beide?
    append_only_log.add_operation(personUuid, operation, dependencies, this.projectUUID + personUuid + newName); //Welche entryID? Ist es Oke newName auch mitzugeben, seitdem auch
  }

  createTask( taskUUID: uuid, personUUID: uuid, title: string, description: string, append_only_log?: AppendOnlyLog): Task {
...
export class Task {//TODO: assignees hinzufügen
  taskUUID: uuid;
  state: number;
  title: string;
  description: string;
